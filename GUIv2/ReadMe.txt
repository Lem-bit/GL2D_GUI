...

{
  ...
  Периодически (когда есть свободное время и желание) компоненты добавляю, обновляю, что то подправляю и дописываю.
  
  К сожалению нормально компоненты никто не тестировал, так что возможны некоторые
  ошибки в работе, если вы нашли проблему, то можете написать мне на почту я постараюсь исправить.
  
  Так же пишите, что можно добавить в компоненты или что исправить для удобства в использовании компонентов.
  
  Самая актуальная версия исходников находится вот тут https://github.com/Lem-bit/GL2D_GUI обновляется чаще чем на сайте
    lemgl.ru
  ...
  - Шрифт сделан текстурами, нельзя установить черный цвет текста (так как черный это прозрачный), можно установить параметры текстуры
    на прозрачные и включить альфа канал, но качество текста будет так себе.  
  - В загрузчике текстур не проверялась загрузка JPG, JPEG, TGA текстур, и загрузка текстур из ресурсов
} 
